...
function DynamicForm() {
  ...
  function handleAddNewQuestion(){
    ...
    if(questions[0]){
      var range = questions.length-1
     
      if(questions[range].quest.length < 10){
        handleShow()
        setModelError('Pergunta está muito curta!')
        return;
      }
    }
  ...
  function handleSendDatas(){
    ...
    .then((element) => {
      ...
      for(i of questions){
        askData.ask = i.quest
        askData.askweight = i.weigth
        askData.formId = element.data.id
        askData.is_image = i.images
